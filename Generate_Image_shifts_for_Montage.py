...
def parse_commandline():
    ...
    parser.add_argument(
        "-tr",
        "--tiltaxisrotation",
        help="Rotation between tilt axis and camera axes (required)",
        required=True,
        type=float,
    )
    ...
    parser.add_argument(
        "-ts",
        "--tiltstep",
        help="Step of tomography tilt series in degrees (3 by default)",
        required=False,
        type=float,
        default=3.0,
    )
    ...
    parser.add_argument(
        "-tm",
        "--maxtilt",
        help="Maximum tilt in tilt series (required)",
        required=True,
        type=float,
    )
    ...
    parser.add_argument(
        "-tg",
        "--dosesymmetrictiltgroup",
        help="Number of tilts in dose-symmetric tilt group (3 by default)",
        required=False,
        type=int,
        default=3,
    )
    ...
    parser.add_argument(
        "-p",
        "--pixelsize",
        help="Pixel size of camera in Angstrom  (required)",
        required=True,
        type=float,
    )
    ...
    parser.add_argument(
        "-n",
        "--camerapixels",
        help="Detector size in pixels (required)",
        required=True,
        nargs=2,
        type=int,
    )
    ...
    parser.add_argument(
        "-M",
        "--montagetiles",
        help="Number of tiles in montage (required)",
        required=True,
        nargs=2,
        type=int,
    )
    ...
    parser.add_argument(
        "-ov",
        "--montageoverlap",
        help="Montage overlap factor (1/overlap fraction), default 10",
        required=False,
        nargs=2,
        type=float,
        default=[10.0, 10.0],
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output basename (default Imageshifts)",
        required=False,
        type=str,
        default="Imageshits",
    )
    ...
    parser.add_argument(
        "-plt",
        "--plot",
        help="Plot imageshifts (default True)",
        required=False,
        action="store_true",
    )
...
if __name__ == "__main__":
    ...
    for itilts in range(ntilts):
        ...
        imagexy = generate_montage_shifts(
            o, [m, n], [nx, ny], shift=[(itilts / o[0]) % 1.0, (itilts / o[1]) % 1.0]
        )
        ...
        if generate_plot:
            ...
            if itilts == 0:
                minx, maxx = [
                    np.amin(imagexy[:, 0]) * p - nx * p,
                    np.amax(imagexy[:, 0]) * p + nx * p * 1.5,
                ]
                miny, maxy = [
                    np.amin(imagexy[:, 1]) * p - ny * p,
                    np.amax(imagexy[:, 1]) * p + ny * p * 1.5,
                ]
            # Plot tilt-axis
            a.plot(
                [minx, maxx],
                [
                    minx * np.tan(np.deg2rad(tiltaxisrotation)),
                    maxx * np.tan(np.deg2rad(tiltaxisrotation)),
                ],
                "k--",
            )
    ...
    write_tilts_and_image_shifts_to_file(
        output + ".txt", tilts, imageshifts, [z / 1e4 for z in zs]
    )
