...
def parse_commandline() -> Dict[str, Any]:
    ...
    parser.add_argument(
        "-i2", "--input2", help=".mrc file for that will be used as alignment reference", required=False, type=str,default='None',
    )
    parser.add_argument(
        "-o",
        "--output",
...
def stitch(ims,positions,msks,pixel_size,binning=1,montagewidth=None,montageorigin=None,smooth=True,renormalize=False):
    ...
    if smooth:
        ...
        canvas[overlap < 1] = np.median(canvas[overlap == 1])
           # fig,ax = plt.subplots()
    # 
    # plt.show(block=True)
        # fig,ax=plt.subplots()
        # p = positions * 1e4 / pixel_size / binning
        # ax.imshow(canvas,cmap='gray',origin='lower')
        # ax.plot(*p.T,'ro',linestyle='-',label='Montage tiles')
...
def montage(
    ...
    refmrcmemmap = mrcfile.mrcmemmap.MrcMemmap(alignfile)
    ...
    refpositions = refpositions[:, :2] * pixel_size * 1e-4

    # fig,ax = plt.subplots()
    # ax.plot(*positions[M].T,'ro',linestyle='-',label='Montage tiles')
    # for i,pos in enumerate(positions[M]):
    #     ax.text(pos[0],pos[1],str(i))
    # plt.show(block=True)

    # Compress positions of reference images to account for relative tilt
    img = copy.deepcopy(mrcmemmap.data[0])
    # print(tilt_axis,relative_tilt)
    # fig,ax = plt.subplots(ncols=2,nrows=2)
    # ax= ax.ravel()
    # ax[0].imshow(img,vmin=np.percentile(img,1),vmax=np.percentile(img,99),cmap='gist_gray')
    
    T = np.eye(2)
    T[1,1] = compression_factor 
    # if relative_tilt >= 0:
    #      np.cos(np.deg2rad(relative_tilt))
    # else:
    #     T[1,1] = 1/np.cos(np.deg2rad(relative_tilt))
    # o = np.asarray(img.shape)
    # M = np.linalg.inv(rotation_matrix(tilt_axis))
    # ax[1].imshow(affine_transform(img,M,offset =o),vmin=np.percentile(img,1),vmax=np.percentile(img,99),cmap='gist_gray')
    # M = np.linalg.inv(T @ rotation_matrix(tilt_axis))
    # ax[2].imshow(affine_transform(img,M,offset =o),vmin=np.percentile(img,1),vmax=np.percentile(img,99),cmap='gist_gray')
    # M = np.linalg.inv(rotation_matrix(-tilt_axis) @ T @ rotation_matrix(tilt_axis))
    # ax[3].imshow(affine_transform(img,M),vmin=np.percentile(img,1),vmax=np.percentile(img,99),cmap='gist_gray')
    # plt.show(block=True)
    T =  rotation_matrix(-tilt_axis) @ T @ rotation_matrix(tilt_axis)
    refpositions = refpositions @  T
    ...
    refmsks = []
    # if binning <2:
    #     refims = refmrcmemmap.data
    # else:
    refims = []
    # Load image from memory map and convert to numpy array
    for  img in tqdm(refmrcmemmap.data,desc='Masking reference images'):
        im = copy.deepcopy(np.asarray(img))
        # Fourier interpolate if reqeusted
        if binning > 1:
            im = fourier_interpolate(im, [x // binning for x in im.shape])
        # Shrink image to account for extra tilt
        im = affine_transform(im,T,output_shape=pixels)
        refims.append(im)
        refmsks.append(make_mask(im, shrinkn=fringe_size / binning,medianthreshold=maskthreshold,absolutethreshold=maskabsolutethreshold))
        # fig,ax = plt.subplots(ncols=2)
        # ax[0].imshow(refims[-1])
        # ax[1].imshow(np.where(refmsks[-1],1,0))
        # plt.show(block=True)
    ...
    distmatrix = cdist(positions[M],refpositions)
    reftile = []
    for overlap in overlaps:
        i, j = overlap
        # See which tile in the reference set minimizes the mutual distance
        k = np.argmin(distmatrix[i]**2+distmatrix[j]**2)
        reftile.append(k)
    ...
    if not skipcrosscorrelation:
        ...
        fileout = os.path.join(
            outdir, os.path.splitext(os.path.split(image)[1])[0] + "_cross_correlation_plots.pdf"
        )
    ...
    msks = clip_masks_to_overlaps(msks,positions[M],overlaps,pixels,pixel_size * binning,ims)
...
def clip_masks_to_overlaps(masks, positions, overlaps, pixels, pixel_size,ims):
    """
    ...
    for i, j in overlaps:
        ...
        # Update the mask of tile i to only include pixels closer to its center
        clipped_masks[i][i1[0] : i1[1], j1[0] : j1[1]] = np.logical_and(closer,clipped_masks[i][i1[0] : i1[1], j1[0] : j1[1]])
        # fig,ax = plt.subplots(ncols=2,nrows=2)
        # ax[0,0].imshow(np.where(clipped_masks[i],distancefromcenter[i],0),cmap='gray')
        # ax[0,1].imshow(np.where(clipped_masks[j],distancefromcenter[j],0),cmap='gray')
        # ax[1,0].imshow(closer,cmap='gray')
        # ax[1,1].imshow(clipped_masks[i][i1[0] : i1[1], j1[0] : j1[1]],cmap='gray')
        # plt.show(block=True)
        closer = np.greater(
            distancefromcenter[i][i1[0] : i1[1], j1[0] : j1[1]],
            distancefromcenter[j][i2[0] : i2[1], j2[0] : j2[1]],
...
def cross_correlate_tiles(
    ...
    for ind, (i, j) in enumerate(tqdm(overlaps, desc="Cross-correlation alignment")):
        ...
        # Calculate relative shift in pixels
        dx1 = [int(x) for x in (x3 - x1) / pixel_size * 1e4][::-1]
        dx2 = [int(x) for x in (x3 - x2) / pixel_size * 1e4][::-1]
        if cross_corr_param_file is None:

            # Perform masked phase cross-correlation to find the shift
            # between both images 1 and 2 and the reference image
            # The shift between images 1 and 2 is then the difference
            ...
            for ii,dx in zip([i,j],[dx1, dx2]):
                ...
                reference_mask = np.zeros_like(masks[ii], dtype=bool)
                reference_mask[i1[0] : i1[1], j1[0] : j1[1]] = np.logical_and(masks[ii][
                    i1[0] : i1[1], j1[0] : j1[1]
                ],refmasks[k][
                    i2[0] : i2[1], j2[0] : j2[1]
                ])
                moving_mask = np.zeros_like(refmasks[k], dtype=bool)
                moving_mask[i2[0] : i2[1], j2[0] : j2[1]] = np.logical_and(masks[ii][
                    i1[0] : i1[1], j1[0] : j1[1]
                ],refmasks[k][
                    i2[0] : i2[1], j2[0] : j2[1]
                ])
                ...
                # aligned_image = stitch(
                #     [tiles[ii], reftiles[k]],
                #     [positions[i], positions[i] + detected_shifts[-1][::-1] * pixel_size * 1e-4],
                #     [masks[ii], refmasks[k]],
                #     pixel_size,
                #     smooth=False,
                #     renormalize=True,
                # )
                # guess_alignment = stitch(
                #     [tiles[ii], reftiles[k]],
                #     [positions[i], positions[i] + np.asarray(dx)[::-1] * pixel_size * 1e-4],
                #     [masks[ii], refmasks[k]],
                #     pixel_size,
                #     smooth=False
                # )
                
                # ax[2+3*m].imshow(aligned_image,cmap='gist_gray')
                # ax[2+3*m].set_title(f'detected shift: {detected_shifts[-1][::-1]} px, Error {error:.4f}')
                # # # ax[1+3*m].imshow(guess_alignment,cmap='gist_gray')
                
                # # # Image.fromarray(aligned_image.astype(np.float32)).save(f'aligned_tile_{ii}_to_ref_{k}.tiff')

                # ax[0+3*m].imshow(tiles[ii],vmin = np.percentile(tiles[ii],5),vmax = np.percentile(tiles[ii],95),cmap='gist_gray')
                # ax[0+3*m].set_title(f'Tile {ii}')
                # ax[1+3*m].imshow(reftiles[k],vmin = np.percentile(reftiles[k],5),vmax = np.percentile(reftiles[k],95),cmap='gist_gray')
                # ax[0+3*m].imshow(reference_mask, alpha=0.1, cmap="Reds")
                # ax[1+3*m].imshow(moving_mask, alpha=0.1, cmap="Reds")
                m+=1
            # plt.show(block=True)
            ...
            detected_shift = np.asarray(detected_shifts[0]) - np.asarray(detected_shifts[1])
        ...
        shifttoolarge = (
            np.linalg.norm((np.asarray(dx1)-np.asarray(dx2)) * pixel_size * 1e-4 - delta) > max_correction
        )
        if genplot:
            x1 = positions[i]
            # delta = (detected_shift * 1e4) * pixel_size
            if shifttoolarge:
                linestyle = "--"
            else:
                linestyle = "-"
    ...
    # fig,ax = plt.subplots()
    # ax.plot(*positions.T,'ro',linestyle='-',label='Montage tiles')
    # ax.plot(*newpositions.T,'bo',linestyle='--',label='Montage tiles')
    # for i,pos in enumerate(positions):
    #     ax.text(pos[0],pos[1],str(i))
    # plt.show(block=True)
...
def main():
    ...
    alignfile = args["input2"]
    if alignfile == 'None':
        match = re.search(r'_(-?[0-9]+(?:\.[0-9]+)?)\.mrc', files[0])
        print(match)
        if match:
            number = float(match.group(1))
        else:
            print('Could not determine alignment tilt from filename, please provide with --input2')
            sys.exit(1)
        
        nexttilt = np.sign(number)*(np.abs(number) +3)
        alignfile = files[0].replace(f'_{number}.mrc', f'_{nexttilt}.mrc')
        print(alignfile)
        if os.path.exists(alignfile):
            print(f'Using alignment tilt file: {alignfile}')
        else:
            prevtilt = np.sign(number)*(np.abs(number) -3)
            alignfile = files[0].replace(f'_{number}.mrc', f'_{prevtilt}.mrc')
            print(alignfile)
            if os.path.exists(alignfile):
                print(f'Using alignment tilt file: {alignfile}')
            else:
    ...
    alignmdocfile = alignfile.replace(".mrc", ".mrc.mdoc")
    m = parse_mdoc(alignmdocfile)
    aligntilt = float(m["ZValue = 0"]["TiltAngle"])
    ...
    indx = find_closest_index(tiltsfromfile, aligntilt)
    refpositions = imageshifts[indx]
    ...
    for i, (file, tilt) in enumerate(
        ...
        if args["maximageshift"] is not None:
            ...
            tiles = None
        # plot_positions(positions[tiles][:,:2],color='k')
        relative_tilt = aligntilt - tilt
        if np.abs(aligntilt) > np.abs(tilt):
            compression_factor = np.cos(np.deg2rad(relative_tilt))
        else:
            compression_factor = 1/np.cos(np.deg2rad(relative_tilt))
        ...
        mont = montage(
            ...
            tilt_axis=extract_tilt_axis_angle(file.replace(".mrc", ".mrc.mdoc" )),
        )
